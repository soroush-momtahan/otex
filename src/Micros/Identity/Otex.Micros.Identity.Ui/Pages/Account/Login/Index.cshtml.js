...
document.addEventListener('alpine:init', () => {
    Alpine.data('otpComponent', () => ({
        digits: ['', '', '', '', '', ''],
        timeLeft: 120, // زمان به ثانیه (2 دقیقه)
        timerInterval: null,
        ...
        onInput(index, event) {
            ...
            // ارسال اتوماتیک اگر هر 6 باکس پر شد
            if (this.digits.every(d => d !== '')) {
                // با تاخیر 50 میلی‌ثانیه سابمیت میکنیم تا کاربر عدد آخر رو ببینه
                setTimeout(() => {
        ...
        onPaste(event) {
            ...
            if (pastedData.length > 0 && pastedData.every(char => /^[0-9]$/.test(char))) {
                ...
                // ارسال اتوماتیک در صورت کامل بودن 6 رقم پیست شده
                if (this.digits.every(d => d !== '')) {
