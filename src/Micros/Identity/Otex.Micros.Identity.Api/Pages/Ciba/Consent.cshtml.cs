...
namespace Otex.Micros.Identity.Api.Pages.Ciba;
    ...
    public async Task<IActionResult> OnPostAsync(CancellationToken ct)
    {
        ...
        var request =
            await _interaction.GetLoginRequestByInternalIdAsync(
                Input.Id ?? throw new ArgumentNullException(nameof(Input.Id)), ct);
        ...
        if (Input.Button == "no")
        {
            ...
            await _events.RaiseAsync(
                new ConsentDeniedEvent(User.GetSubjectId(), request.Client.ClientId,
        ...
        else if (Input.Button == "yes")
        {
            ...
            if (Input.ScopesConsented.Any())
            {
                ...
                // emit event
                await _events.RaiseAsync(
                    new ConsentGrantedEvent(User.GetSubjectId(), request.Client.ClientId,
                        request.ValidatedResources.RawScopeValues, result.ScopesValuesConsented, false), ct);
